...
const afSlots: QuestionSlot[] = [
  {
    ...
    variants: [
      { variantId: "sfx-af-pf-mcq", step: { type: "mcq", question: "An email says you've won $500 000 in a lottery you never entered. To claim it you must pay R2 500 in 'release fees'. What is this?", options: ["Advance fee fraud", "A legitimate international lottery", "A tax requirement on foreign winnings", "A processing charge you'll get back"], correct: 0, feedback: { correct: "Right. There's no prize. Pay the R2 500 and a new fee appears (customs, legal, transfer) until you stop.", incorrect: "It's advance fee fraud. You can't win a lottery you never entered, and the 'fee' is the scam." } } },
  ...
  },
  {
    ...
    variants: [
      ...
      { variantId: "sfx-af-fa-mcq", step: { type: "mcq", question: "A 'SARS official' demands R15 000 immediately, payable in gift-card vouchers, or you'll be arrested. What's happening?", options: ["A scam, no department takes vouchers", "A legitimate urgent collection", "A penalty arrangement", "A standard SARS process for old arrears"], correct: 0, feedback: { correct: "Right. Vouchers are untraceable, which is exactly why scammers ask for them and no real authority ever does.", incorrect: "Voucher demands are always fraud. SARS collects through recognised channels, not iTunes cards." } } },
      { variantId: "sfx-af-fa-sc", step: { type: "scenario", question: "Thabo gets a threatening call claiming to be from a government department. How does he verify it?", options: ["Hang up and look up the number", "Ask the caller for their number", "Trust it, they knew his ID number", "Pay a small amount to test it"], correct: 0, feedback: { correct: "Right. Numbers can be spoofed and personal details are widely leaked. An independently sourced number is the only real check.", incorrect: "Hang up and call back on a number you find yourself. Anything the caller gives you is part of the script." } } },
    ],
  },
  {
    ...
    variants: [
      ...
      { variantId: "sfx-af-js-mcq", step: { type: "mcq", question: "Besides upfront fees, which sign most reliably marks a fake job offer?", options: ["A huge salary with no interview", "A fully remote working arrangement", "An immediate start date", "Communication only by email"], correct: 0, feedback: { correct: "Right. Check the company on the CIPC register and look for a real physical address and phone number.", incorrect: "The implausible salary with no process behind it. Remote work and quick starts are ordinary." } } },
    ],
  },
  {
...
const vishSlots: QuestionSlot[] = [
  ...
  },
  {
    ...
    variants: [
      ...
      { variantId: "sfx-vs-kd-mcq", step: { type: "mcq", question: "Why do vishing callers open with accurate personal information?", options: ["To build credibility before the ask", "Because only the bank could have it", "To comply with the regulations", "To confirm your identity for you"], correct: 0, feedback: { correct: "Right. The accurate details are the setup; the request for an OTP or a transfer is the payload.", incorrect: "It's a credibility play. Personal data circulates widely after breaches." } } },
      { variantId: "sfx-vs-kd-sc", step: { type: "scenario", question: "A caller correctly states Johan's recent transactions. What should he conclude?", options: ["Nothing yet, he should still hang up", "The caller must surely be genuine then", "His account is compromised by an insider", "He should just cooperate with them fully"], correct: 0, feedback: { correct: "Right. Verification means calling back on a number he sources himself, whatever the caller knows.", incorrect: "Detailed knowledge isn't verification. Hang up and call the bank on your own number." } } },
    ],
  },
  {
...
const waSlots: QuestionSlot[] = [
  {
    ...
    variants: [
      { variantId: "sfx-wa-im-sc", step: { type: "scenario", question: "An unknown number messages: 'Hi, it's Lungelo from varsity. I'm stuck in Cape Town, lost my wallet, please EFT R1 500.' Best response?", options: ["Call the real Lungelo yourself", "Send the money, he's in trouble", "Reply asking for his bank details", "Ask a security question by message"], correct: 0, feedback: { correct: "Right. A voice call to the number you already hold defeats the entire scam in thirty seconds.", incorrect: "Verify by voice on your existing number. A stranger claiming a familiar name is the whole trick." } } },
  ...
  },
  {
    ...
    variants: [
      ...
      { variantId: "sfx-wa-gi-mcq", step: { type: "mcq", question: "Why is 'my cousin has been paid twice' weak evidence for an investment scheme?", options: ["Early Ponzi participants are paid", "Cousins are simply unreliable", "It shows unregistered advice", "Payments are always reversed later"], correct: 0, feedback: { correct: "Right. Early payouts are the recruitment tool, not proof of a working investment.", incorrect: "Early payouts are how Ponzi schemes recruit. They prove the scheme is running, not that it works." } } },
    ],
  },
  {
    ...
    conceptId: "whatsapp-scams",
    variants: [
      { variantId: "sfx-wa-qr-mcq", step: { type: "mcq", question: "A seller sends a QR code to 'confirm you're receiving payment'. What's really happening?", options: ["Scanning it pays money out, not in", "A standard way to receive your money", "It verifies your identity", "It generates a receipt"], correct: 0, feedback: { correct: "Right. QR codes initiate payments. Nobody needs a code from you to send you money. Your number or account details are enough.", incorrect: "Scanning a QR code sends money. Receiving money never requires you to scan anything." } } },
  ...
  },
  {
    ...
    variants: [
      ...
      { variantId: "sfx-wa-at-sc", step: { type: "scenario", question: "Thabo's WhatsApp is taken over and his contacts are being asked for money. What should he do first?", options: ["Re-register the number to reclaim it", "Wait for it to be restored automatically", "Buy a brand new SIM", "Reply to the scammer from another number"], correct: 0, feedback: { correct: "Right. Re-registering logs the attacker out, and an urgent warning limits the damage to his contacts.", incorrect: "Re-register immediately and warn people through another channel. Every hour costs someone money." } } },
    ],
  },
];
...
const waAppSlots: QuestionSlot[] = [
  ...
  },
  {
    ...
    conceptId: "investment-scams",
    variants: [
      { variantId: "sfx-ws-rc-mcq", step: { type: "mcq", question: "The scheme pays a bonus for every friend you bring in. What does that tell you?", options: ["Income comes from recruitment", "A generous referral programme", "A registered marketing structure", "It reduces your own risk"], correct: 0, feedback: { correct: "Right. When new money pays old members, the scheme collapses the moment recruitment slows, and the last people in lose everything.", incorrect: "Recruitment-based income is the Ponzi signature. Real investments earn from markets, not from members." } } },
      ...
      { variantId: "sfx-ws-rc-sc", step: { type: "scenario", question: "Lerato is asked to bring five people into the group to 'unlock' her withdrawal. What's happening?", options: ["Her money is leverage to recruit more", "A standard membership requirement here", "A security verification step", "A tax compliance rule"], correct: 0, feedback: { correct: "Right. The withdrawal will never come, and the condition exists only to extend the scheme's life.", incorrect: "It's recruitment leverage. No legitimate investment ties your withdrawal to bringing in others." } } },
    ],
  },
  {
    ...
    variants: [
      { variantId: "sfx-ws-vf-mcq", step: { type: "mcq", question: "Before putting money into any scheme, what's the minimum check?", options: ["Verify the FSP number on the register", "Ask members whether they've been paid", "Check how many people are in it", "Read all the testimonials"], correct: 0, feedback: { correct: "Right. Not on the register means no licence, no oversight and no complaints route. That alone ends the conversation.", incorrect: "Independent verification on the FSCA register. Members and testimonials are part of the pitch." } } },
  ...
  },
  {
    ...
    conceptId: "scam-recovery",
    variants: [
      { variantId: "sfx-ws-wd-sc", step: { type: "scenario", question: "Sipho put in R100 000 and received R40 000 in 'returns' within two months, with pressure to reinvest and recruit. What's the safest action?", options: ["Withdraw what he can and report it", "Reinvest to compound the returns", "Recruit friends to reduce exposure", "Wait and see if payments continue"], correct: 0, feedback: { correct: "Right. Those payments are other people's capital. Withdrawing early is the only outcome he controls, and recruiting makes him part of their loss.", incorrect: "Get out and stop recruiting. Reinvesting hands back the only money he's likely to see." } } },
      { variantId: "sfx-ws-wd-mcq", step: { type: "mcq", question: "Where should an investment scheme be reported in South Africa?", options: ["The FSCA, and SAPS", "The NCR", "SARS", "The Reserve Bank"], correct: 0, feedback: { correct: "Right. The FSCA regulates financial services conduct; SAPS handles the fraud itself.", incorrect: "The FSCA for the financial conduct, SAPS for the crime. The NCR handles credit, not investments." } } },
