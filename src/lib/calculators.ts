...
export type GrowthInputs = {
  ...
  escalation: number;
  frequency: Frequency;
  /** Annual product fee / EAC (%). Applied monthly as a balance drag. */
  fee?: number;
  /** Annual inflation (%). Does not change the nominal path; used for realValue. */
  inflation?: number;
  /** Monthly withdrawal in rands, starting at withdrawFromYear. */
  withdrawal?: number;
  /** First year (0-based) withdrawals begin. Defaults to years (none in the horizon). */
  withdrawFromYear?: number;
  wrapper?: AccountWrapper;
  /** Gross annual income — used only to size the RA contribution cap. */
  annualIncome?: number;
  /** Lifetime TFSA contributions already used (rands). */
 ...
 * Annual fee is a monthly balance drag: balance *= (1 + r/12) * (1 - f/12).
 * Annual mode adds the year's contribution as a year-end lump after 12 months of compounding.
 * Escalation steps the contribution up once per year-end.
 * Withdrawals (if any) start at withdrawFromYear; contributions stop that year.
 * Living-annuity withdrawals are clamped to 2.5%–17.5% of the year-start balance.
 */
export function projectGrowth(inputs: GrowthInputs): GrowthProjection {
  ...
  for (let year = 0; year <= n.years; year++) {
    ...
    const drawing = year >= n.withdrawFromYear && currentWithdrawal > 0;
    ...
    let yearWithdrawalBudget = drawing ? currentWithdrawal * 12 : 0;
    ...
    if (n.inflation > 0) currentWithdrawal *= 1 + n.inflation / 100;
  }

  if (depletedYear !== null) {
    notes.push(`The pot reaches R0 in year ${depletedYear}.`);
  }

  const uniqueNotes = [...new Set(notes)];
  return { points, notes: uniqueNotes, depletedYear, annualContributionCap: cap };
}
...
function accumulationBase(base: GrowthInputs): GrowthInputs {
  return { ...base, withdrawal: 0, withdrawFromYear: MAX_GROWTH_YEARS };
}
...
/** Smallest whole month where the accumulation path meets the goal. */
...
export function solveForMonthly(base: GrowthInputs, goal: number): number {
  ...
  if (growthFinal({ ...clean, monthly: cap }).value < goal) {
...
export function solveForWithdrawal(base: GrowthInputs, surviveYears?: number): number {
  ...
  const survives = (w: number) => {
    const p = projectGrowth({ ...base, years, withdrawal: w, withdrawFromYear: startYear });
...
export function assumptionLine(inputs: GrowthInputs): string {
  ...
  if (n.withdrawal > 0 && n.withdrawFromYear < n.years) {
    bits.push(`withdrawals from year ${n.withdrawFromYear}`);
  }
