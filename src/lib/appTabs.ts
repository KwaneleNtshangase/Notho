...
export type AppTabKey = "learn" | "calculator" | "budget" | "quests" | "profile";
...
export function tabKeyFromPath(pathname: string): AppTabKey {
  return normalizeTabPath(pathname).slice(1) as AppTabKey;
}

export function tabKeyFromHref(href: string): AppTabKey | null {
  if ((APP_TAB_HREFS as readonly string[]).includes(href)) {
    return href.slice(1) as AppTabKey;
  }
  return null;
}
